...
import { app } from '../../app';
import { TabViewItem } from '../../../model/TabViewItem';
...
export interface TabNaviViewProp extends ViewProps {
    /**
     * 数据列表
     */
 ...
 * 底部标签导航
 */
export class TabNaviView extends Component<TabNaviViewProp, TabNaviViewState> {


    listCache = Array<any>();
    constructor(props: TabNaviViewProp) {
        super(props);
        let st = new TabNaviViewState();
    ...
    renderItem(dat: TabViewItem, index: number) {
        ...
        return (
            <PressView key={index} onPress={() => this.onPressItem(dat, index)}
                style={[{ flex: 1, paddingVertical: this.props.itemPaddingV ?? 10 }, styles.center]} >
