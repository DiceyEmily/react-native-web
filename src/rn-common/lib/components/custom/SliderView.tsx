...
export type SliderProps = {
  ...
  showDot?: boolean;
  step?: number;
  //限制onValueChange的触发频率，毫秒
  timeLimit?: number;
  minimumTrackTintColor?: string;
  maximumTrackTintColor?: string;
  allowTouchTrack?: boolean;
  thumbTintColor?: string;
  punctuationStyleColor?: string;
  ...
  punctuationStyle?: StyleProp<ViewStyle>;
  thumbProps?: any;
...
> {
  ...
  getPropValue() {
    return this.props.state ? this.props.state[0] : this.props.value
  }
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      value: new Animated.Value(
        getBoundedValue(this.getPropValue(), props.maximumValue, props.minimumValue)
      ),
    };
    ...
    this.panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (e, g) => this.handlePanResponderGrant(e, g),
  ...
  timeLimit = this.props.timeLimit == null ? 0 : this.props.timeLimit;
  ...
  handlePanResponderGrant(e: GestureResponderEvent, gestureState: PanResponderGestureState) {
    ...
    if (this.thumbHitTest(e)) {
      let v = this.getCurrentValue();
      let left = this.getThumbLeft(v);
    ...
    let v = this.getValue(gestureState)
    this.setCurrentValue(v);


    this.fireChangeEvent('onValueChange');
    if (this.props.state)
      this.props.state[1](v)
  }
  ...
  handlePanResponderMove(_, gestureState) {
    ...
    if (Date.now() - this.lastChange < this.timeLimit) {
      return;
    }
  ...
  handleStartShouldSetPanResponder(e /* gestureState: Object */) {
    ...
    if (!this.props.allowTouchTrack) {
      return this.props.trackPressable || this.thumbHitTest(e);
    }
...

        {
          this.props.showDot ? [
            ...
            <SliderThumb
              ...
              style={punctuationStyle}
              color={punctuationStyleColor}
              start={(containerSize.width - 10) * 1 / 3}
              {...thumbProps}
            />,
            <SliderThumb
              key={2}
              style={punctuationStyle}
              color={punctuationStyleColor}
              start={(containerSize.width - 10) * 2 / 3}
              {...thumbProps}
            />,
            <SliderThumb
              key={3}
