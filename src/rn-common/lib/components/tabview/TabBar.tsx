...
> {
  ...
  render() {
    ...
    let indicatorLeft = 0;
    if (indicatorStyle && "width" in indicatorStyle && !isWidthDynamic && typeof indicatorStyle.width === "number") {
      const scale = lib.getScale();
      let w = indicatorStyle.width;
      // console.log(tabBarWidth, w, (window as any)?._set_font_size)
      indicatorLeft = ((tabBarWidth / routes.length - w * scale) / 2) * 100 / tabBarWidth;
    }
      ...
      >
        <Animated.View
          ...
          style={[
            ...
            scrollEnabled ? { transform: [{ translateX }] as any } : { marginLeft: indicatorLeft + "%" },
            tabBarWidth
              ? { width: lib.getPx(tabBarWidth) }
              : scrollEnabled
        ...
        <View style={styles.scroll}>
          <Animated.ScrollView
            ...
            contentContainerStyle={[
              styles.tabContent,
              scrollEnabled
                ? { width: lib.getPx(tabBarWidth) || tabBarWidthPercent }
                : styles.container,
              contentContainerStyle,
            ]}
