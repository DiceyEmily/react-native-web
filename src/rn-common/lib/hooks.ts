 ...
 * 包含$update属性的状态
 */
export type WithUpadte<T> = T & (UpadteFunc);
 ...
 * @returns 
 */
export function useObject<T, PropT>(func: () => T, prop?: PropT): WithUpadte<WithProp<T, PropT>> {
 ...
 * @returns 
 */
export function useObjState<T, PropT>(func: () => T, prop?: PropT, toStateMap = true): WithUpadte<WithProp<T, PropT>> {
...
// export function useAsync(func: () => Promise<void | (() => Promise<void>)>, deps: DependencyList) {
    // let funcRef = useRef(func)
    // useEffect(() => {
